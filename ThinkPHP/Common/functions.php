...
function showMessage($info,$url=null)//输出提示信息
{
	if($url)
	{
		echo "<meta charset='UTF-8'><script>alert('$info');window.location.href='$url';</script>";
	}else
	{
		echo "<meta charset='UTF-8'><script>alert('$info');location=document.referrer;  </script>";
	}
	exit;
}

function changeState ($state)  //启用或禁用管理员
{
	if ($state=='q')
	{
		$status = 1;
...
function userEdit ($state,$id)//判断用户评论状态
{
	if ($state==1)
	{
		$message = "<a href='".__CONTROLLER__."/edit/id/$id/type/j'>禁言</a>";
	}else 
	{
		$message = "<a href='".__CONTROLLER__."/edit/id/$id/type/q'>取消禁言</a>";
	}
	return $message;
}


function isEmpty ($data) //判断图文上传中表单是否有空值
{
	if ($data['na_ad_name']==null||$data['na_ad_type']==null||$data['na_ad_place']==null||$data['na_ad_content']==null)
		showMessage('所有信息不能为空');
}

function isType ($a,$b)//类型匹配
{
	if ($a==$b)
	{
		echo "selected";
